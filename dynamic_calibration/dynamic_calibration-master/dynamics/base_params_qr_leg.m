function [pi_lgr_base, baseQR] = base_params_qr_leg(includeMotorDynamics)
% ----------------------------------------------------------------------
...
rng('shuffle');
includeMotorDynamics=0;
% ------------------------------------------------------------------------
% Set limits on posistion and velocities
% ------------------------------------------------------------------------
q_min = -pi*ones(4,1);
q_max = pi*ones(4,1);
qd_max = 3*pi*ones(4,1);
q2d_max = 6*pi*ones(4,1); 

% -----------------------------------------------------------------------
% Standard dynamics paramters of the robot in symbolic form
% -----------------------------------------------------------------------
m = sym('m%d',[4,1],'real');
hx = sym('h%d_x',[4,1],'real');
hy = sym('h%d_y',[4,1],'real');
hz = sym('h%d_z',[4,1],'real');
ixx = sym('i%d_xx',[4,1],'real');
ixy = sym('i%d_xy',[4,1],'real');
ixz = sym('i%d_xz',[4,1],'real');
iyy = sym('i%d_yy',[4,1],'real');
iyz = sym('i%d_yz',[4,1],'real');
izz = sym('i%d_zz',[4,1],'real');
im = sym('im%d',[4,1],'real');
...
W = [];    
for i = 1:25
    q_rnd = q_min + (q_max - q_min).*rand(4,1);
    qd_rnd = -qd_max + 2*qd_max.*rand(4,1);
    q2d_rnd = -q2d_max + 2*q2d_max.*rand(4,1);
    
    if includeMotorDynamics
        Y = regressorWithMotorDynamics(q_rnd,qd_rnd,q2d_rnd);
    else
        Y = full_regressor_leg(q_rnd,qd_rnd,q2d_rnd);
    end
    W = vertcat(W,Y);
end
